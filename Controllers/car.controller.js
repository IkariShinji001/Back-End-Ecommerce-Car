...
const carController = {
  async getCars(req, res) {
    ...
    try {
      const cars = await Cars.find(filterQuery)
        .limit(limit)
        .skip(limit * (page - 1))
        .sort({ name: 1 });
  ...
  async getTotalCar(req, res) {
    try {
      const totalCar = await Cars.aggregate([{ $group: { _id: null, count: { $sum: 1 } } }, { $project: { _id: 0, count: 1 } }]);
  ...
  async updateCar(req, res) {
    ...
    try {
      ...
      await Cars.findByIdAndUpdate(carId, { $push: { 'description.color': { $each: color } } });
