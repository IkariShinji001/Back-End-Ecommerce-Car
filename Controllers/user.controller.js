...
const UserController = {
    ...
    async updateUserInforById(req, res){
        ...
        if (updatedData.hasOwnProperty('username') || updatedData.hasOwnProperty('password') || 
            updatedData.hasOwnProperty('role')) {
            return res.status(400).json({ error: "Không được phép cập nhật username hoặc password hoặc role"});
        }
        try{
